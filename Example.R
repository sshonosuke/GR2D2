...
SNR <- 0.7      # SNR (signal-to-noise ratio)

if(p%%10!=0){ print("Please set 'p' to be a multiple of 10.") }
G <- p/10
p_G <- rep(10, G)
ID <- rep(1:G, p_G)
...
Fit[[5]] <- R2D2_group(Y, X, Group_ID=ID, phi_dist="Dir", mc=mc, bn=bn)   # grouped R2D2 (Dirichlet)
Fit[[6]] <- R2D2_group(Y, X, Group_ID=ID, phi_dist="LN", mc=mc, bn=bn)    # grouped R2D2 (Logistic normal)

Fit[[6]]$ac_ratio      # acceptance rate 
